...
const SettingsModal = ({ isOpen, onClose, initialCategory = 'profile', appConfig = {}, setConfig, theme, initApp }) => {
    ...
    const renderContent = () => {
        switch (activeCategory) {
            case 'profile':
                return <ProfileSettings handleImport={handleImport} handleExport={handleExport} />;
            case 'config':
                return <GlobalConfigSettings appConfig={appConfig} installedApps={installedApps} initApp={initApp} />;
            case 'ssh':
                return <SSHManagementSettings sshSessions={sshSessions} />;
            default:
                return (
