...
const ServiceIdentity: React.FC<any> = ({ service, renderIcon }) => (
  <div className="flex items-center gap-3">
    ...
    <h3 className="text-base font-black text-slate-900 dark:text-white uppercase italic tracking-tighter">
      {service.name}
    </h3>
  </div>
);

const VersionSwitcher: React.FC<any> = ({
  service,
  task,
  handleSwitchVersion,
}) => {
  ...
  return (
    <div className="flex items-center gap-1 ml-1">
      {installedVersions.map((ver: string) => {
        const systemString = (service.activeVersion || "")
          .toString()
          .toLowerCase()
          .trim();
        const cleanVer = ver
        ...
        return (
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              handleSwitchVersion(service.name, ver);
            }}
            className={cn(
  ...
  <div className="flex items-center gap-3">
    {!isInstalled && service.name !== "OpenSSL" ? (
      <button
        ...
        className="px-4 py-1.5 bg-blue-600/10 hover:bg-blue-600/20 text-blue-600 dark:text-blue-400 rounded-sm text-[9px] font-black uppercase tracking-widest transition-all border border-blue-500/20"
      >
        Install First
      </button>
    ) : (
      <button
        ...
        onClick={(e) => {
          e.stopPropagation();
          handleToggleService(service.name, service.status);
        }}
        className={cn(
        ...
        {isTransitioning ? (
          <div className="absolute inset-0 flex items-center justify-center">
            <Loader2
              ...
              className={cn(
                "animate-spin",
                service.status === "Running" ? "text-white" : "text-blue-500",
              )}
            />
          </div>
        ) : (
    ...
    <button
      ...
      onClick={(e) => {
        e.stopPropagation();
        handleRemoveFromHome(service.name);
      }}
    ...
    <div className="flex items-center flex-wrap gap-4 px-1 pb-2">
      ...
      {hasOpenFolder && (
        <button
          ...
          className="w-8 h-8 flex items-center justify-center bg-slate-100 dark:bg-white/5 hover:bg-blue-600/10 text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 rounded-sm border border-slate-200 dark:border-white/5 transition-all"
          title="Open Folder"
        >
          <FolderOpen size={16} />
        </button>
      ...
      {hasTerminal && (
        <div className="relative">
          <button
            ...
            onClick={() =>
              setOpenTerminalDropdown(
                openTerminalDropdown === service.name ? null : service.name,
              )
            }
            className={cn(
          ...
          {openTerminalDropdown === service.name && (
            <>
              <button
                type="button"
                className="fixed inset-0 z-[150] w-full h-full bg-transparent border-none p-0 cursor-default focus:outline-none"
                onClick={() => setOpenTerminalDropdown(null)}
              />
              <div className="absolute top-full left-0 mt-1 w-40 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-sm shadow-2xl z-[160] animate-in fade-in slide-in-from-top-1 duration-200">
                <div className="p-1">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      handleOpenLocalTerminal(service.name, "powershell")
                    }
                    className="w-full flex items-center gap-3 px-3 py-1.5 rounded-sm text-[10px] font-bold text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5 hover:text-slate-900 dark:hover:text-white transition-all text-left"
                  >
...
const ServiceItem: React.FC<ServiceItemProps> = ({
  ...
  const isInstalled =
    (task?.installedVers && task.installedVers.length > 0) ||
    service.name === "OpenSSL";
  ...
  const hasTerminalFacility =
    service.name !== "HeidiSQL" && service.name !== "OpenSSL";
  const hasOpenFolder =
    isInstalled && service.name !== "OpenSSL" && service.name !== "HeidiSQL";
