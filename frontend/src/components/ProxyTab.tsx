...
const ProxyTab: React.FC<ProxyTabProps> = ({ addToast }) => {
  ...
  const handlePortChange = (name, value) => {
    setApps((prev) =>
      prev.map((app) =>
        app.name === name ? { ...app, port: Number.parseInt(value) || 0 } : app,
      ),
    );
  };
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <div
                      className={`w-2 h-2 rounded-full ${proxyStatuses[app.name] ? "bg-emerald-500 animate-pulse" : "bg-slate-300 dark:bg-slate-700"}`}
                      title={proxyStatuses[app.name] ? "Online" : "Offline"}
                    ></div>
                  </div>
              ...
              <div className="relative">
                <button
                  ...
                  onClick={() =>
                    setOpenTerminalDropdown(
                      openTerminalDropdown === app.name ? null : app.name,
                    )
                  }
                ...
                {openTerminalDropdown === app.name && (
                  <>
                    <button
                      type="button"
                      className="fixed inset-0 z-10 w-full h-full bg-transparent border-none p-0 cursor-default focus:outline-none"
                    ...
                    <div className="absolute top-full right-0 mt-1 w-40 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-lg shadow-xl z-20 animate-in fade-in slide-in-from-top-1 duration-200 overflow-hidden">
                      <div className="p-1">
                        <button
                          type="button"
                          onClick={() => {
                            AppBackend.OpenProxyTerminal(app.name, "cmd");
                            setOpenTerminalDropdown(null);
                          }}
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="mt-2 text-[10px] text-slate-400 italic">
                  Points {app.name}.test to http://127.0.0.1:
                  {app.port || "____"}
                </p>
              </div>
            </div>
      ...
      <div className="mb-6 flex justify-between items-center">
        <div>
          <p className="text-slate-500 dark:text-slate-400 text-sm">
            Configure local proxy pass for folders in your WWW directory.
          </p>
        </div>
        <div className="relative">
          <Search
