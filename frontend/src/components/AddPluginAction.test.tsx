...
describe('AddPluginAction Component', () => {
  ...
  it('calls setIsAddingPlugin when clicking the main toggle button', () => {
    ...
    render(<AddPluginAction {...defaultProps} setIsAddingPlugin={setIsAddingPlugin} />)
  ...
  it('shows a message when all plugins are already pinned', () => {
    ...
    expect(screen.getByText('All plugins are already pinned')).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /MySQL/i })).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /PHP/i })).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /Nginx/i })).not.toBeInTheDocument()
  })
})
