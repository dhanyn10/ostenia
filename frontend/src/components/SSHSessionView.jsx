...
const SSHSessionView = ({ session, onClose, addToast, isActive, theme }) => {
 ...
 } catch (err) { addToast('Error', 'Download failed: ' + err, 'error'); }
 };
 ...
 } catch (err) { addToast('Error', 'Upload failed: ' + err, 'error'); }
 };
 ...
 loadRemoteFiles(remotePath);
 } catch (err) { addToast('Error', 'Edit failed: ' + err, 'error'); }
 };
 ...
 loadRemoteFiles(remotePath);
 } catch (err) { addToast('Error', 'Delete failed: ' + err, 'error'); }
 }
 };
 ...
 <button onClick={() => { handleDelete(fileContextMenu.file); setFileContextMenu(null); }} className="w-full px-4 py-2 text-left text-[11px] font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 flex items-center gap-2">
