...
describe('Icons Component', () => {
  describe('RawSVGIcon (Icons.Raw)', () => {
    ...
    it('renders with the correct style masks and size when svgString is provided', () => {
      ...
      const { container } = render(<Icons.Raw svgString={svgString} size={24} className="test-class" />)
      ...
      expect(div.style.webkitMaskImage).toContain(`data:image/svg+xml,${encodedSvg}`)
    ...
    it('cleans <?xml ?> tag from svgString', () => {
      const svgString = '<?xml version="1.0" encoding="UTF-8"?><svg><rect /></svg>'
