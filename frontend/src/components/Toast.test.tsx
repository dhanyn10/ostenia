...
describe("Toast Component", () => {
  ...
  it("applies correct classes based on toast type", () => {
    ...
    const { container } = render(
      <Toast toasts={mockToasts} removeToast={removeToast} />,
    );
    ...
    expect((toastElements?.[0] as HTMLElement).className).toContain(
      "bg-emerald-50",
    );
    expect((toastElements?.[1] as HTMLElement).className).toContain(
      "bg-rose-50",
    );
  });
