...
func (o *Orchestrator) updateServiceInfo(name string) ServiceDetailedInfo {
	...
	if name == "Node.js" {
		...
		if _, err := os.Stat(nodeExe); err == nil {
			cmd := exec.Command(nodeExe, "-v"); cmd.SysProcAttr = &syscall.SysProcAttr{HideWindow: true}
			out, _ := cmd.Output(); info.ActiveVersion = string(out)
		}
	...
	if name == "PHP" || name == "Apache" || name == "MySQL" || name == "Nginx" {
		...
		if name == "PHP" {
			...
			if _, err := os.Stat(phpExe); err == nil {
				cmd := exec.Command(phpExe, "-v"); cmd.SysProcAttr = &syscall.SysProcAttr{HideWindow: true}
				out, _ := cmd.Output(); info.ActiveVersion = string(out)
			}
	...
	for _, line := range lines {
		...
		if len(fields) >= 5 && fields[len(fields)-1] == pidStr {
			...
			if lastColon != -1 {
				...
				if p > 0 {
					exists := false
					for _, existing := range ports { if existing == p { exists = true; break } }
