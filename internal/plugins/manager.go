...
func (m *Manager) DeleteVersion(taskName, version string) error {
	if runtime.GOOS == "windows" {
		m.killProcessesForTask(taskName)
	}
	dir := m.getTaskVersionDir(taskName, version)
	return os.RemoveAll(dir)
}

func (m *Manager) killProcessesForTask(taskName string) {
...
func (m *Manager) getTaskVersionDir(taskName, version string) string {
...
func (m *Manager) DownloadAndExtract(task DownloadTask) error {
	...
	if m.isTaskAlreadyInstalled(task, targetDir) {
		_ = m.ensureCurrentLink(task)
		m.emit(m.ctx, "download_progress", Progress{Name: task.Name, Percentage: 100, Status: "Ready"})
		return nil
	}

	tmpFile, err := m.downloadWithCancel(task)
	if err != nil {
		return err
	}

	if !strings.HasSuffix(strings.ToLower(task.URL), ".zip") && !strings.HasSuffix(strings.ToLower(task.URL), ".nupkg") {
		return m.runInstaller(tmpFile, targetDir, task.Name)
	}

	return m.extractAndLink(tmpFile, targetDir, task)
}

func (m *Manager) isTaskAlreadyInstalled(task DownloadTask, targetDir string) bool {
...
func (m *Manager) downloadWithCancel(task DownloadTask) (string, error) {
	ctx, cancel := context.WithCancel(m.ctx)
	m.registerCancel(task.Name, cancel)
	defer m.unregisterCancel(task.Name, cancel)

	ext := m.getFileExtension(task.URL)
...
func (m *Manager) registerCancel(name string, cancel context.CancelFunc) {
	m.cancelsMu.Lock()
	m.cancels[name] = cancel
	m.cancelsMu.Unlock()
}

func (m *Manager) unregisterCancel(name string, cancel context.CancelFunc) {
	cancel()
	m.cancelsMu.Lock()
	delete(m.cancels, name)
	m.cancelsMu.Unlock()
...
func (m *Manager) getFileExtension(url string) string {
	lowerURL := strings.ToLower(url)
	if strings.HasSuffix(lowerURL, ".zip") {
		return ".zip"
	}
	if strings.HasSuffix(lowerURL, ".nupkg") {
		return ".nupkg"
	}
	return ".exe"
}

func (m *Manager) runInstaller(tmpFile, targetDir, taskName string) error {
	...
	m.emit(m.ctx, "download_progress", Progress{Name: taskName, Percentage: 100, Status: "Completed"})
	return nil
}

func (m *Manager) extractAndLink(tmpFile, targetDir string, task DownloadTask) error {
	...
	if err := m.unzipFile(m.ctx, tmpFile, extractTmp, task.Name); err != nil {
		m.emit(m.ctx, "download_progress", Progress{Name: task.Name, Percentage: 0, Status: "Error: " + err.Error()})
		return err
	}

	m.normalizeExtractedDir(extractTmp, strings.HasSuffix(strings.ToLower(task.URL), ".nupkg"))
...
func (m *Manager) normalizeExtractedDir(dir string, isNupkg bool) {
	if isNupkg {
		toolsDir := filepath.Join(dir, "tools")
		if _, err := os.Stat(toolsDir); err == nil {
			m.moveFiles(toolsDir, dir)
	...
	if len(es) == 1 && es[0].IsDir() {
		sDir := filepath.Join(dir, es[0].Name())
		m.moveFiles(sDir, dir)
		_ = os.Remove(sDir)
	}
}

func (m *Manager) moveFiles(src, dest string) {
	es, _ := os.ReadDir(src)
	for _, e := range es {
...
func (m *Manager) unzipFile(ctx context.Context, src, dest, name string) error {
	...
	for i, f := range r.File {
		if err := m.unzipSingleFile(ctx, f, dest, name, i, total); err != nil {
			return err
		}
	}
	return nil
...
func (m *Manager) unzipSingleFile(ctx context.Context, f *zip.File, dest, name string, index, total int) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	default:
		...
		if index%20 == 0 {
			m.emit(m.ctx, "download_progress", Progress{Name: name, Percentage: (float64(index+1) / float64(total)) * 100, Status: "Extracting..."})
		}
		return nil
	}
}
